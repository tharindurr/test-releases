...
const PRERELEASE_NAME = 'prerelease'
...
async function main () {
  registerVersioningStrategy('only-minor', () => {
    class _OnlyMinor {
      ...
      determineReleaseType (version, commits) {
        core.info(`OnlyMinorVersionUpdate: ${JSON.stringify(version)}`)
        return new OnlyMinorVersionUpdate()
      }
    }
    return new _OnlyMinor()
  })

  registerVersioningStrategy('only-patch', () => {
    class _OnlyPatch {
      bump (version, commits) {
        return this.determineReleaseType(version, commits).bump(version)
      }
      ...
      determineReleaseType (version, commits) {
        core.info(`OnlyPatchVersionUpdate: ${JSON.stringify(version)}`)
        return new OnlyPatchVersionUpdate()
      }
    }
    return new _OnlyPatch()
  })

  registerVersioningStrategy('only-pre-release', () => {
    class _OnlyPrerelease {
      bump (version, commits) {
        return this.determineReleaseType(version, commits).bump(version)
      }

      determineReleaseType (version, commits) {
        core.info(`PrereleaseVersionUpdate: ${JSON.stringify(version)}`)
